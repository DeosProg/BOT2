...
def callback(call):
    ...
    if call.message:
        ...
        #ДОМАШНЕЕ ЗАДАНИЕ-------------------------------------------------------
        if call.data == '0':
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id,text='На какой день вы хотите узнать домашнее задание?',reply_markup=markup3)
        elif call.data == '1':
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id,text='На какой день вы хотите узнать домашнее задание?',reply_markup=markup31)
        elif call.data == '01':
            try:
                lst = homework0.Mo
                ...
                bot.send_document(call.message.chat.id,open(r'/home/deos/BOT/'+urls[0], 'rb'))
                print(lst)
            except Exception as exc:
                traceback.print_exc()
        elif call.data == '02':
        ...
        #РАСПИСАНИЕ-------------------------------------------------------------
            ...
            try:
                ...
                try:
                    ...
                    place = 'нигде'
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.id,text=texts.timetables_text.format(
                          day=day + month,
                          place=place,
                          pair1=pairs[0],
                          pair2=pairs[1],
